...
<head>
    ...
    <style>
        ...
        .items img {
            ...
            /* Add border radius only to the left side */
        }
        ...
        .point.active {
            background-color: #4caf50;
            /* Green color for active bars and points */
        }
        ...
        .bar.active {
            ...
            /* 2s duration, ease timing function, fill forwards */
        }
...
<body>
    ...
    <div class="container">
        ...
        JOIN tbl_order_details AS pre ON o.fk_preid = pre.id
        JOIN tbl_products AS p ON pre.product_id = p.product_id
        WHERE o.user_id = '$userid' order by o.order_date desc";
                    ...
                    ?>
                        <div class="item" data-product-id="<?php echo $row['product_id']; ?>">
                            ...
                            <div class="item-content">
                                ...
                                <div id="mo" style="display:flex;">
                                <span class="label" style="display:block;"> Order Accepted </span>
                                <span class="label" style="display:block;"> Order Processed</span>
                                <span class="label" style="display:block;"> Order Shipping </span>
                                <span class="label" style="display:block;"> Order Delivered</span>

                                </div>
                                <p><?php echo $statusLabels[$status - 1]; ?></p>
                                <p>Delivery Date: <?php echo date('F j, Y', strtotime($row['order_date'])); ?></p>
                                <p>Delivery Time: <?php echo 'Instant' ?></p>
                                <p>Created At: <?php echo date('F j, Y h:i A', strtotime($row['order_date'])); ?></p>
                                ...
                                <?php } elseif ($paymentCompleted) { ?>
                                    <button id="paid">Paid</button>
                                    <?php
                                                                    $isScheduled = isDeliveryScheduled($order_id);
                                                                    if (!$isScheduled) {
                                    ?>
                                    <button id="sch" onclick="openModal(<?php echo $row['order_id']; ?>)">Schedule Delivery</button>
<?php }?>
<!-- The Modal -->
<div id="myModal" class="modal">
    ...
    <div class="modal-content">
        ...
        <form id="deliveryForm">
            ...
            <button id=sch type="submit">Schedule Delivery</button>
        </form>
    </div>
</div>
                                ...
                                $rating_inserted = isRatingInserted($userid, $order_id);
                                if ($status == 4 && !$rating_inserted) {
                                    ?>
                                    <div class="rating" id="rating-<?php echo $row['product_id']; ?>" data-orderid="<?php echo $row['order_id']; ?>" data-productid="<?php echo $row['product_id']; ?>">
                                        <?php
        ...
        function isRatingInserted($userid, $order_id) {
            global $conn;
            $sql = "SELECT * FROM tbl_ratings WHERE user_id = '$userid' AND order_id = '$order_id'";
            $result = $conn->query($sql);
            return ($result->num_rows > 0);
        }
        function isDeliveryScheduled($order_id)
        {
            global $conn;
            $sql = "SELECT * FROM tbl_schedule WHERE order_id = '$order_id'";
            $result = $conn->query($sql);
            return ($result->num_rows > 0);
        }
        ?>
    ...
    ratings.forEach(function(rating) {
        ...
        stars.forEach(function(star) {
            star.addEventListener("click", function() {
                ...
                var productId = rating.getAttribute("data-productid");
                var orderId = rating.getAttribute("data-orderid");
                ...
                xhr.send("product_id=" + productId + "&order_id=" + orderId + "&rating=" + ratingValue);

// Alert user
alert('You rated ' + ratingValue + ' stars for product ' + productId + ' in order ' + orderId);


                // Remove any existing filled class from stars in this rating container
                stars.forEach(function(s) {
                    s.classList.remove("filled");
                });

                // Add filled class to stars up to the clicked star
...
document.getElementById('deliveryForm').addEventListener('submit', function(event) {
    ...
    // Send the schedule delivery request via AJAX
    scheduleDelivery(orderId, fromDate, toDate);
});

function scheduleDelivery(orderId, fromDate, toDate) {
        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                Swal.fire({
                    ...
                    text: 'Your delivery has been scheduled successfully!',
                ...
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Error scheduling delivery. Please try again.',
                    confirmButtonText: 'OK'
                });
            }
        }
    ...
    xhr.send('orderId=' + encodeURIComponent(orderId) + '&fromDate=' + encodeURIComponent(fromDate) + '&toDate=' + encodeURIComponent(toDate));
}
