...
JOIN tbl_schedule AS s ON o.order_id = s.order_id JOIN tbl_order_details as od ON od.id=o.fk_preid JOIN tbl_products as p ON p.product_id=od.product_id  WHERE p.fk_regid='$userid'   order by created_at desc";
...
<p>From Date: <?php echo date("F j, Y h:i A", strtotime($row['from_date'])); ?></p>
<p>created_at: <?php echo date("h:i A", strtotime($row['created_at'])); ?></p>
<p>To Date: <?php echo date("F j, Y h:i A", strtotime($row['to_date'])); ?></p>
<?php
...
function updateStatus(orderId, status) {
    ...
    xhr.open('POST', 'schedule_update.php', true);
